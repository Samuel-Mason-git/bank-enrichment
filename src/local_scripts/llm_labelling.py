...
- Judge new-parent creation purely on fit, never on how established or convenient the closest existing parent is. If a transaction only fits an existing parent because it's loosely, thematically related — not because it's genuinely the same kind of thing that parent already represents — a new parent is NOT a worse answer than that stretch-fit. Only prefer an existing parent when the transaction is a genuine match for what it already represents, not merely adjacent to it.
- Output ONLY the raw JSON array. No analysis, no reasoning, no markdown. Just the JSON.
